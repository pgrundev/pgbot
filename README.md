...
| Method | Command |
|---|---|
| npx / bunx / pnpm dlx (no install) | `npx @pgbot/cli inspect "$DATABASE_URL"` |
...
The wrapper locates the binary at run time and pgbot ships **no install scripts**,
...
there is no `node_modules/.bin`, so invoke it as `yarn pgbot`.
